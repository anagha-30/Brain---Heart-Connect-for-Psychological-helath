...
def BuildKFold(data,n=10,features=pd.DataFrame(),Biosignal_Option='both'):
    ...
    if Biosignal_Option=='eeg':
        ...
        X = data.loc[:,'ECG_Rate_Mean':'HRV_SampEn'].to_numpy()
    elif Biosignal_Option=='both':
        X = data.loc[:,'psdtheta_1':'HRV_SampEn'].to_numpy()
...
def driver():
    ...
    if(action=="preprocessing"):
        ...
        Results = [] # I renamed your previous "results" into "Results" because there is a conflict between it and the argparser's results
